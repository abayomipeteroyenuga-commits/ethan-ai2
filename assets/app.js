...
function isStandaloneOffice(){
  return window.matchMedia?.('(display-mode: standalone)').matches===true || navigator.standalone===true;
}
function updateInstallUI(){
  const b=document.getElementById('installBtn');
  if(!b)return;
  if(window.EthanAndroid||isStandaloneOffice()){
    b.textContent='Ethan Office Installed';
    b.disabled=true;
    b.classList.remove('hidden');
  }else{
    b.textContent='Install Ethan Office';
    b.disabled=false;
    b.classList.remove('hidden');
  }
}
...
window.addEventListener('appinstalled',()=>{
  installPrompt=null;
  updateInstallUI();
  try{toast('Ethan Office installed successfully.')}catch(e){}
});
window.requestOfficeInstall=async function(){
  if(window.EthanAndroid||isStandaloneOffice()){
    try{toast('Ethan Office is already installed on this device.')}catch(e){}
    return;
  }
  if(installPrompt){
    try{
      ...
      if(choice?.outcome==='accepted'){
        ...
        try{toast('Installation was cancelled. You can install Ethan Office anytime.')}catch(e){}
      }
      return;
    }catch(e){console.error('PWA install prompt',e)}
  }
  const secure=location.protocol==='https:'||location.hostname==='localhost'||location.hostname==='127.0.0.1';
  const ios=/iphone|ipad|ipod/i.test(navigator.userAgent);
  const msg=!secure
    ? 'For browser installation, Ethan Office must be opened from its HTTPS website. Upload this PWA to your secure domain, then reopen it there.'
    : ios
      ? 'On iPhone/iPad, open Ethan Office in Safari, tap Share, then choose Add to Home Screen.'
      : 'Your browser has not offered its install prompt yet. In Chrome/Edge, open the browser menu and choose Install app or Add to Home screen. If Ethan Office was already installed, open it from your apps.';
  if(typeof modal==='function'){
    modal('Install Ethan Office',`<p>${msg}</p><p class="muted">The web version supports installation only when the browser considers the PWA eligible.</p>`,()=>closeModal());
    const ms=document.getElementById('modalSave');if(ms)ms.textContent='Close';
  }else alert(msg);
};
...
// ---------- UI injection ----------
function inject(){
 const tb=$('#wordWorkspace .toolbar');if(tb&&!$('#wordStats')){tb.insertAdjacentHTML('beforeend',`<button onclick="wordCmd('undo')">Undo</button><button onclick="wordCmd('redo')">Redo</button><button onclick="wordCmd('insertOrderedList')">1. List</button><button onclick="wordCmd('justifyRight')">Right</button><button onclick="wordCmd('justifyFull')">Justify</button><select onchange="wordCmd('fontSize',this.value)"><option value=3>Font</option><option value=2>Small</option><option value=3>Normal</option><option value=5>Large</option><option value=7>XL</option></select><input type=color title="Text color" onchange="wordCmd('foreColor',this.value)"><input type=color title="Highlight" onchange="wordCmd('hiliteColor',this.value)"><button onclick="wordInsertLink()">Link</button><button onclick="wordInsertTable()">Table</button><button onclick="wordFindReplace()">Find/Replace</button><button onclick="wordVersionHistory()">Versions</button><span id=saveStatus class=saveStatus>Saved locally</span>`);$('#wordEditor').addEventListener('input',()=>{updateWordStats();status('Unsaved changes',false);autoSaveWord()});$('#wordTitle').addEventListener('input',()=>{status('Unsaved changes',false);autoSaveWord()})}
 ...
 const fm=$('#page-files .titleRow');if(fm&&!$('#fileSearch'))fm.insertAdjacentHTML('afterend',`<div class=panel style="padding:12px;margin-bottom:14px"><input id=fileSearch placeholder="Search files" oninput="renderFiles()" style="width:100%"></div><div class=section><div class=titleRow><h2>Recycle Bin</h2><button class="btn red sm" onclick="emptyTrash()">Empty Bin</button></div></div>`);
 const top=document.querySelector('.top-actions');if(top&&!$('#networkStatus'))top.insertAdjacentHTML('afterbegin',`<button class=btn onclick="showInstallHelp()">Install Help</button>`);
 const hero=document.querySelector('#page-home .hero div');if(hero&&!$('#installCard'))hero.insertAdjacentHTML('beforeend',`<div id=installCard class=installCard><b>Use it like an app</b><span>Install Ethan Office from your browser for a home-screen icon and offline access.</span><button class="btn" onclick="requestOfficeInstall()">Install</button><button class="btn" onclick="showInstallHelp()">How?</button></div>`);
...
window.wordInsertPageBreak=()=>{document.execCommand('insertHTML',false,'<p><br></p>');autoSaveWord()};
...
function ethanBrandSanitize(value=''){
  return String(value)
    .replace(/easy(?:\s|&nbsp;|&#160;|[-_])*office(?:\s|&nbsp;|&#160;|[-_])*suite/gi,'Ethan Office Suite')
    .replace(/easy(?:\s|&nbsp;|&#160;|[-_])*office/gi,'Ethan Office')
    .replace(/easyoffice/gi,'Ethan Office');
}
...
window.sheetChart=(type='bar')=>{const s=state.sheets.find(x=>x.id===currentSheet);if(!s)return;const labels=s.grid.slice(0,12).map((r,i)=>String(r[0]||`Row ${i+1}`)),vals=s.grid.slice(0,12).map(r=>Number(r[1])||0);const max=Math.max(...vals,1);let body='<div style="padding:15px"><h3>Quick '+type+' chart — Column B</h3>';if(type==='pie'){body+='<div class=muted>Pie-style share view</div>'+vals.map((v,i)=>`<div style="margin:7px 0">${esc(labels[i])}: <b>${v}</b> (${Math.round(v/(vals.reduce((a,b)=>a+b,0)||1)*100)}%)</div>`).join('')}else body+=vals.map((v,i)=>`<div style="display:flex;align-items:center;gap:8px;margin:7px 0"><span style="width:90px">${esc(labels[i])}</span><b>${v}</b></div>`).join('');body+='</div>';modal('Chart Preview',body,()=>closeModal());$('#modalSave').textContent='Close'};
...
window.wordBuildIndex=function(){const ed=$('#wordEditor');const marks=[...ed.querySelectorAll('.wordIndexEntry[data-index-term]')];if(!marks.length){toast('Mark index entries first');return}const map={};marks.forEach(el=>{const t=el.dataset.indexTerm.trim();(map[t]??=[]).push(approxPageFor(el))});const rows=Object.keys(map).sort((a,b)=>a.localeCompare(b)).map(t=>`<div class="indexRow"><span>${esc(t)}</span><span>${[...new Set(map[t])].join(', ')}</span></div>`).join('');document.execCommand('insertHTML',false,`<section class="wordIndex"><h2>Index</h2>${rows}</section>`);autoSaveWord();toast('Index created')};
...
window.wordBibliography=function(){const d=currentWordDoc();ensureWordMeta(d);if(!d.sources.length){toast('No saved sources');return}const li=d.sources.map(x=>`<li>${esc(x.author)} (${esc(x.year)}). <i>${esc(x.title)}</i>.</li>`).join('');document.execCommand('insertHTML',false,`<section class="wordBibliography"><h2>Bibliography</h2><ol>${li}</ol></section>`);autoSaveWord()};
...
(function(){
  ...
  // Installation is handled by the v20 PWA installer above.
...
(function(){
 ...
 try{
   ...
   DB.save();
 }catch(e){console.error('v18 stability initialization',e)}
})();
...
(function(){
  ...
  try{
    ...
    const clean=typeof ethanBrandSanitize==='function'?ethanBrandSanitize:(v=>String(v??''));
    (state.wordDocs||[]).forEach(d=>{
      d.title=clean(d.title||'Untitled Document');
      d.content=clean(d.content||'');
      d.header=clean(d.header||'');
      d.footer=clean(d.footer||'');
      (d.versions||[]).forEach(v=>{
        if(v?.payload){
          if('title'in v.payload)v.payload.title=clean(v.payload.title);
          if('content'in v.payload)v.payload.content=clean(v.payload.content);
          if('header'in v.payload)v.payload.header=clean(v.payload.header);
          if('footer'in v.payload)v.payload.footer=clean(v.payload.footer);
        }
      });
    });
    (state.files||[]).forEach(f=>{if(f?.name)f.name=clean(f.name)});
    (state.recents||[]).forEach(r=>{if(r?.title)r.title=clean(r.title);if(r?.name)r.name=clean(r.name)});
    DB.save();
